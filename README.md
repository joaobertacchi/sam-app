...
* AWS CLI already configured with Administrator permission
  * Create aws account
  * Create a user for CLI admin
    * Access IAM service via AWS console and select User tab
    * Click add user
    * Chose a user name (e.g. adminuser) and select only *Programmatic access* in *access type*. Click next
    * In *Set permissions* choose *Attach existing policies directly* and select *AdministratorAccess* policy
    * Click next until user is created
    * Download .csv file. It contains your user *Access key ID* and *Secret access key*
  * Install AWS CLI
    * `pip3 install awscli`
  * Configure AWS CLI
    * `aws configure`

    Use *Access key ID* and *Secret access key* from user you just created
...
* SAM CLI
  ...
  * Adjust your PATH to include the Python scripts that are installed under the user's home directory
  ...
  See [install instructions](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-cli-install.html) for additional information.
